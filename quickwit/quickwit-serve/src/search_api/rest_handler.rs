...
pub(crate) async fn extract_index_id_patterns(
    ...
    info!("search target index is {}", &comma_separated_index_patterns);
    let index_pattern = decode(&comma_separated_index_patterns)
        .expect("the index_pattern should url decode")
        .to_string();
...
mod tests {
    ...
    async fn test_extract_index_id_patterns() {
        ...
        assert_eq!(
            extract_index_id_patterns("my-index-1%2Cmy-index-2%2A".to_string())
                .await
                .unwrap(),
            vec!["my-index-1".to_string(), "my-index-2*".to_string()]
        );
